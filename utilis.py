...
# ----------------------------
def fetch_dataset():
    logger.info("Starting fetch_dataset...")
...
def build_or_load_vector_store(results):
    logger.info("Entering build_or_load_vector_store...")
...
def get_llm(temperature: float = 0.2):
    if not check_ollama_connection():
        logger.warning("Ollama might not be reachable. LLM calls may hang or fail.")
    
    logger.info(f"Initializing OllamaLLM with model='llama2', temperature={temperature}")
    return OllamaLLM(model="llama2", temperature=temperature)
